...
function showToast(msg, dur = 2500) {
  // FIX 3: Strip emoji from toast messages for clean UI
...
// FIX 1: openModal/closeModal updated to use 'active' class
// to match index.html modal system exactly
function openModal(id) {
  const el = document.getElementById(id);
  if (!el) return;
  // Support both 'show' (exam/study/results pages) and 'active' (home page)
...
function toggleTheme() {
  ...
  // Also support data-theme attribute used in some pages
  document.documentElement.setAttribute('data-theme', isLight ? 'dark' : 'light');
  localStorage.setItem('medquiz_theme', isLight ? 'dark' : 'light');
  updateThemeIcon();
}
...
function updateThemeIcon() {
  ...
  // SVG moon icon for dark mode, sun icon for light mode
...
function saveAIConfig() {
  ...
  showToast(
    aiConfig.provider === 'pollinations'
      ...
      : (aiConfig.apiKey ? 'Gemini AI enabled!' : 'AI disabled — no Gemini key set')
  );
}
...
async function callAI(prompt) {
  ...
  if (provider === 'gemini') {
    if (!aiConfig.apiKey) throw new Error('Gemini API key not set. Click the star icon to add one, or switch to Pollinations (free).');
  ...
  const body  = {
    model,
    messages: [
      { role: 'system', content: 'You are an expert medical educator and tutor. Provide clear, accurate, concise explanations.' },
  ...
  const sample = [
    {
      ...
      "explanation": "Fine, persistent froth indicates active respiration in water. Other findings are less specific.",
      "difficulty": 3, "subject": "Forensic Medicine",
      "topic": "Asphyxial Deaths", "tags": ["PYQ", "HYT"]
    },
    {
      "question": "Which best describes rigor mortis?",
      "options": ["Postmortem cooling", "Muscle stiffening due to ATP depletion", "Skin discoloration", "Soft tissue decomposition"],
      "correct_option": "Muscle stiffening due to ATP depletion",
      "explanation": "Rigor mortis is postmortem muscle stiffening from ATP depletion, beginning 2-6 hours after death.",
      "difficulty": 2, "subject": "Forensic Medicine",
      "topic": "Postmortem Changes", "tags": ["HYT"]
    },
    {
      "question": "The cherry-red color of blood in CO poisoning is due to:",
      "options": ["Carboxyhemoglobin", "Methemoglobin", "Sulfhemoglobin", "Oxyhemoglobin"],
      "correct_option": "Carboxyhemoglobin",
      "explanation": "CO binds hemoglobin with 240x affinity forming carboxyhemoglobin, giving cherry-red color.",
      "difficulty": 3, "subject": "Forensic Medicine",
      "topic": "Poisoning", "tags": ["PYQ"]
    }
  ];
...
function clearInput() {
  // This is the programmatic version — confirmClearInput() in index.html
  // handles the confirmation dialog version
  const input = document.getElementById('jsonInput');
...
// Internal: hide everything after clear
function _hideAllConfigSections() {
  ...
  // FIX 1: Use class-based reveal — remove 'revealed' instead of display:none
    ...
    try {
      ...
      const converted = rows.map((row) => ({
        ...
        options: [
          ...
          row.option_e || row.OptionE || row['Option E'] || ''
        ].filter(Boolean),
      ...
      if (converted.length === 0) {
        showToast('No valid questions found in file.');
        return;
      }
      const input = document.getElementById('jsonInput');
      if (input) {
      ...
      showToast('Error reading file. Ensure it has columns: question, option_a–d, correct_option.');
    }
  };
...
function setupOCRZone() {
  ...
  zone.addEventListener('drop',      (e) => {
    ...
    if (file && file.type.startsWith('image/')) {
      ocrFile = file;
      showOCRPreview(file);
      document.getElementById('ocrConvertBtn').disabled = false;
    }
  });
}
...
// ═══════════════════════════════════════════════
// FIX 1 + 3 + 4 + 6 + 7: JSON PARSING (Home)
// ═══════════════════════════════════════════════
function parseJSON() {
  ...
  try {
    ...
    if (valid.length === 0) {
      _showParseStatus(status, 'error', `No valid questions found. ${errors.length} error(s): ${errors.slice(0, 3).join(' | ')}`);
    ...
    // FIX 3: NO emoji in status — clean SVG injected by index.html event listener
    // We just set plain text here; the event handler in index.html upgrades it
    let statusMsg = `${valid.length} question${valid.length > 1 ? 's' : ''} loaded successfully`;
    if (errors.length) statusMsg += ` (${errors.length} skipped due to errors)`;
    _showParseStatus(status, 'success', statusMsg);

    // FIX 4: renderPreview without question chip list
    renderPreview();

    // FIX 1: Reveal quiz config using CLASS — not display:block
    // Do NOT call modeSelector.style.display = 'block' directly
    // Instead, fire the custom event which triggers revealQuizConfig() in index.html
    ...
    // Hide start button until mode is chosen
    const startSection = document.getElementById('startSection');
    if (startSection) startSection.style.display = 'none';

    // Hide timer/scoring until exam mode is chosen
...
// FIX 3: Clean status display — no emoji
function _showParseStatus(el, type, msg) {
  if (!el) return;
  el.className       = `parse-status ${type}`;
  el.textContent     = msg;
  el.style.display   = 'block';
  el.style.removeProperty('display'); // allow CSS to control after setting text
}

// FIX 4: renderPreview — NO question chip list, just stats
function renderPreview() {
  ...
  // FIX 4: Clear chip list and keep it hidden — no question listing
  ...
  // FIX 6: Stats bar — clean, no avg difficulty text (meter handles it)
  if (statsBar) {
    statsBar.innerHTML = `
      <div class="stat-item">
      ...
      <div class="stat-item">
        <div class="stat-val">${withDiff.length > 0 ? avgDiff : '—'}</div>
        <div class="stat-lbl">Avg. Difficulty</div>
      </div>`;
  }
...
// Apply shuffle + question target to a question array
function _applyShuffleAndTarget(qs) {
  let result = qs.slice();

  // FIX 5: Shuffle questions if toggle is on
  if (window._shuffleQuestions) {
    result = _shuffleArray(result);
  }

  // FIX 5: Shuffle answer options per question if toggle is on
  if (window._shuffleOptions) {
    result = result.map(q => {
      const shuffledOptions = _shuffleArray(q.options);
      return { ...q, options: shuffledOptions };
    });
  }

  // FIX 7: Apply question target
...
// ═══════════════════════════════════════════════
// FIX 5 + 7: START QUIZ — applies shuffle & target
// ═══════════════════════════════════════════════
function startQuiz() {
  if (!selectedMode) {
    showToast('Please select a quiz mode first.');
    return;
  }

  if (selectedMode === 'retake') {
    startRetake();
    return;
  }
  ...
  if (filtered.length === 0) {
    showToast('No questions match the active filter.');
    return;
  }

  // FIX 5 + 7: Apply shuffle and question target
  filtered = _applyShuffleAndTarget(filtered);

  if (filtered.length === 0) {
    showToast('No questions available after applying settings.');
    return;
  }
  ...
  // Persist to localStorage for exam/study pages
...
async function saveToBank() {
  ...
  showToast(`"${name}" saved to bank (${questions.length} questions).`);
  loadBankList();
}
...
async function loadBankList() {
  ...
  list.innerHTML = banks.map(b => `
    <div class="bank-item">
      ...
      <span class="bank-item-meta">${b.count} questions &middot; ${new Date(b.savedAt).toLocaleDateString()}</span>
...
function viewFolderContent(folder, container) {
  ...
  container.innerHTML = folder.questions.map((q, i) =>
    `<div class="bank-item">
      <span style="font-size:11px;flex:1;min-width:100px">
        ${q.question.substring(0, 50)}...
      </span>
      <button class="btn btn-red btn-xs"
        onclick="removeFromFolder('${folder.folderName}', ${i})">Remove</button>
    </div>`
  ).join('');
}
...
function selectExamOption(opt, el) {
  ...
  updateNavGrid();
  document.getElementById('examAnsweredText').textContent =
    `${Object.keys(examAnswers).length} Answered · ${flagged.size} Flagged`;
}
...
function renderStudyQuestion() {
  ...
  if (aiExplainBtn) {
    aiExplainBtn.style.display = (aiConfig.provider === 'pollinations' || aiConfig.apiKey)
      ? 'inline-flex' : 'none';
  }
...
function showResults() {
  ...
  if (aiAssessBtn) {
    aiAssessBtn.style.display = (aiConfig.provider === 'pollinations' || aiConfig.apiKey)
      ? 'inline-flex' : 'none';
  }
...
function saveRetakeData(wrongResults) {
  const qs = wrongResults.map(r => r.q);
  localStorage.setItem('medquiz_retake_questions', JSON.stringify(qs));
}
...
function downloadResults() {
  ...
  const html = `<!DOCTYPE html><html lang="en"><head><meta charset="UTF-8">
    <title>MedQuiz Pro — Test Report</title>${style}
    </head><body>
...
document.addEventListener('DOMContentLoaded', () => {
  ...
  // ── Expose shuffle/target globals (for index.html toggles) ──
  ...
  if (isHome) {
    ...
    // FIX 1: Ensure quizConfigSection starts hidden via class
    // (no display:none — animation controlled by CSS)
    const configSection = document.getElementById('quizConfigSection');
    if (configSection) {
      configSection.style.removeProperty('display');
      // Only add 'revealed' if it already has questions (e.g., back-button scenario)
      if (questions.length === 0) {
        configSection.classList.remove('revealed');
      }
    }
  }
  ...
  // ── Keyboard shortcuts (exam page only) ─────
  if (isExam) {
